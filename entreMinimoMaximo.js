...
function estaEntre(n, min, max, inclusivo = false){
    if(inclusivo == true){
        if(n >= min && n <= max){
            return `O ${n} está entre ${min}-${max}`  
        } else {
            return `O ${n} não está no periodo ${min}-${max} `
        }
    } else {
        if(n > min && n < max){
            return `O ${n} está entre ${min}-${max}`  
        } else {
            return `O ${n} não está entre o periodo ${min}-${max} `
        }
    }
}

valores = [1, 1, 10, ]
resultadoValores = estaEntre(valores[0], valores[1], valores[2], valores[3])
console.log(valores)
